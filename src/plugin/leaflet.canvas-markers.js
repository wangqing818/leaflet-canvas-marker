...
function layerFactory(L) {
    ...
    var CanvasIconLayer = (L.Layer ? L.Layer : L.Class).extend({
        ...
        onAdd: function (map) {
            ...
            if (!this._imageCanvas) this._initImageCanvas();
        ...
        _addMarker: function(marker, latlng, isDisplaying) {
            ...
            var adj_x = self._calcAdjacentX(marker);
            var adj_y = self._calcAdjacentY(marker);
            var ret = [(self._calcCoords(pointPos, adj_x, adj_y, marker)),({
                minX: latlng.lng,
                minY: latlng.lat,
                maxX: latlng.lng,
                maxY: latlng.lat,
            ...
            if(isDisplaying) self._drawMarker(marker, pointPos, adj_x * 2, adj_y * 2);

            return ret;
        },

        _drawMarker: function (marker, pointPos, width, height) {
            var self = this;
            // self.changeMarkerIcon(marker.options.icon);
            ...
            } else if (this.options.type == 'arc'){
                self._drawArc(pointPos);
            } else if (this.options.type === 'rect'){
                self._drawRect(marker, width, height);
            }
        ...
        changeMarkerIcon(icon){
            if ('iconUrl' in icon.options) return;

            var images = this.canvasWrapText(icon.options.html);
        ...
        _initImageCanvas(){
            ...
            let settings = {
                // canvas:, //canvas对象，必填，不填写默认找到页面中的第一个canvas
                canvasWidth:260, //canvas的宽度
              //   drawStartX:0, //绘制字符串起始x坐标
              //   drawStartY:0, //绘制字符串起始y坐标
                lineHeight:20, //文字的行高
                font:"20px 'Microsoft Yahei'", //文字样式
                // text: text + '', //需要绘制的文本
              ...
              //获取2d的上线文开始设置相关属性
              this._imageCanvas.width = settings.canvasWidth;
              this._imageCanvas.height = settings.lineHeight;
              this._imageCtx = this._imageCanvas.getContext("2d");
              this._imageCtx.font = settings.font;
        },
        ...
        canvasWrapText(text) {
            ...
            //根据arr的长度设置canvas的高度
            var centerWidth = 130 - this._imageCtx.measureText(text).width / 2;
            this._imageCtx.fillText(text, centerWidth, 20);
            return this._imageCanvas.toDataURL();
          },
        ...
        _drawRect(marker, width, height){
            this._context.fillStyle = marker.color;
            let pointPos = this._map.latLngToContainerPoint(marker.buttomLeft);
            this._context.fillRect(pointPos.x, pointPos.y, width, height)
        },

        _drawText(marker, pointPos){
            
            this._context.fillText(
                marker.text,
                pointPos.x - marker.textWidth / 2,
                pointPos.y - this.options.height
            )

        },

        _drawArc: function(pointPos){
            // this._context.beginPath()
            this._context.arc(pointPos.x, pointPos.y, this.options.radius, 0, Math.PI * 2, false)
            this._context.fill();
        },
        ...
        _redraw: function (clear) {
            ...
            self._latlngMarkers.search(mapBoxCoords).forEach(function (e) {
                ...
                //Readjust Point Map
                var pointPos = self._map.latLngToContainerPoint(e.data.latLng);

                // if (e.data.fontWidth === undefined)
                //     e.data.fontWidth = self._context.measureText(e.data.text).width;

                var adj_x = self._calcAdjacentX(e.data);
                var adj_y = self._calcAdjacentY(e.data);
                ...
                var newCoords = self._calcCoords(pointPos, adj_x, adj_y, e.data);

                tmp.push(newCoords);

                //Redraw points
                self._drawMarker(e.data, pointPos, adj_x * 2, adj_y * 2);
            });
        ...
        _calcCoords: function(pointPos, adj_x, adj_y, marker){
            return {
                minX: (pointPos.x - adj_x),
                minY: (pointPos.y - adj_y),
                maxX: (pointPos.x + adj_x),
        ...
        _calcAdjacentY: function(marker) {
             if (this.options.type == 'text'){
                if (marker.textWidth === undefined) marker.textWidth = this._context.measureText(marker.text).width;
                return marker.textWidth / 2;
             }else if (this.options.type == 'arc'){
                return this.options.radius;
            }else if (this.options.type == 'rect'){
                return (this._map.latLngToContainerPoint(marker.buttomLeft).y - this._map.latLngToContainerPoint(marker.topRight).y) / 2;
            }
        },

        _calcAdjacentX: function(marker){
            if (this.options.type == 'text'){
                return this.options.height / 2;
            }else if (this.options.type == 'arc'){
                return this.options.radius;
            }else if (this.options.type == 'rect'){
                if (marker.buttomLeft == undefined){
                    marker.buttomLeft = this._addLatLng(-this.options.deltY / 2, -this.options.deltX / 2, marker.latLng);
                    marker.topRight = this._addLatLng(this.options.deltY / 2, this.options.deltX / 2, marker.latLng);
                }
                return (this._map.latLngToContainerPoint(marker.topRight).x - this._map.latLngToContainerPoint(marker.buttomLeft).x) / 2;
            }
        },

        _addLatLng(latNum, lngNum, latLng){

            return L.latLng([latLng.lat + latNum, latLng.lng + lngNum]);
            // return {
            //     lat: latlng.lat + latNum,
            //     lng: latLng.lng + lngNum
            // }
        ...
        _initContext: function(){
            if (this.options.type === 'text'){
                this._context.font = this.options.font;
                this._context.fillStyle = this.options.color;
            } else if (this.options.type === 'arc'){
                this._context.fillStyle = this.options.color;
            }
        },
